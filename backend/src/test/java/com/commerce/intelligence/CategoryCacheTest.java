...
class CategoryCacheTest {
  ...
  void cacheHitAvoidsDatabaseRead() {
    ...
    when(values.get("catalog:categories:v1")).thenReturn("[]");
  ...
  void malformedCacheEntryIsReplacedWithBoundedTtl() {
    ...
    var repository = mock(CategoryRepository.class);
    when(redis.opsForValue()).thenReturn(values);
    when(values.get("catalog:categories:v1")).thenReturn("invalid json");
    ...
    verify(values).set("catalog:categories:v1", "[]", Duration.ofSeconds(60));
    cache.invalidate();
    verify(redis).delete("catalog:categories:v1");
  }
}
